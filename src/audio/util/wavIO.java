...
*/
	...
	public String toString() {
		return "wavIO [myPath=" + myPath + ", myChunkSize=" + myChunkSize
				+ ", mySubChunk1Size=" + mySubChunk1Size + ", myFormat="
				+ myFormat + ", myChannels=" + myChannels + ", mySampleRate="
				+ mySampleRate + ", myByteRate=" + myByteRate
				+ ", myBlockAlign=" + myBlockAlign + ", myBitsPerSample="
				+ myBitsPerSample + ", myDataSize=" + myDataSize + "]";
	}
	...
	public static boolean bytesToWav(String path, byte[] samples, int bits, int channels, int sampleRate)
	{
		DataOutputStream outFile = null;
		try
		{
			outFile = new DataOutputStream(new FileOutputStream(path));

			// write the wav file per the wav file format
			outFile.writeBytes("RIFF");					// 00 - RIFF
			outFile.write(intToByteArray((int)(samples.length+36)), 0, 4);		// 04 - how big is the rest of this file?
			...
			outFile.write(intToByteArray((int)2*sampleRate), 0, 4);		// 28 - bytes per second           block align * sample rate I think
			outFile.write(shortToByteArray((short)2), 0, 2);	// 32 - # of bytes in one sample, for all channels          block align, bits per sample / 2 rounded up
			outFile.write(shortToByteArray((short)bits), 0, 2);	// 34 - how many bits in a sample(number)?  usually 16 or 24
			outFile.writeBytes("data");					// 36 - data
			outFile.write(intToByteArray((int)samples.length), 0, 4);		// 40 - how big is this data chunk
			outFile.write(samples);						// 44 - the actual data itself - just a long string of numbers
		}
		catch(Exception e)
		{
			e.printStackTrace();
